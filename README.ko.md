...
| 명령 | 동작 |
|------|------|
| `socrates` / `socrates list` | fzf 세션 목록. **Enter → 액션 메뉴**; **Ctrl-Y/Ctrl-O는 복사 후에도 picker 유지** (`--resume` 복사 / `cd … && claude --resume …` 전체 복사 / UUID만 / 별명 / 뒤로); 단축키: **Ctrl-P 선택 세션의 프로젝트로 좁히기**, Ctrl-O cd+resume, Ctrl-Y UUID, Ctrl-N 별명. Ctrl-U/D preview 스크롤, Ctrl-E preview 확대 |
...
| `socrates mem <검색어|id>` | claude-mem 플러그인이 나에 대해 기억하는 내용을 읽기 전용 검색; id를 주면 전체 기록과 **공식 제거 절차**까지 출력 (식별 → 안내; Socrates가 직접 지우지는 않음) |
| `socrates map` | 설정 계층·hooks·plugins·MCP·skills/agents 현황을 터미널에 출력 |
| `socrates report` | 탭형 HTML 대시보드 생성+열기 (Overview / Projects / Sessions / **Config X-ray** / **Memory & Identity** / **Injection** / Harness, **EN/한국어 토글** 지원). Memory & Identity는 Claude가 나를 어떻게 인식하는지(로컬 `~/.claude.json` 계정 정보)와 전 프로젝트의 자동 메모리 파일·설명을 보여줍니다. X-ray는 프로젝트별로 설정 레이어와 **CLAUDE.md 체인** — 루트→프로젝트로 내려오며 세션 시작 시 실제 로드되는 모든 메모리 파일([공식 규칙](https://code.claude.com/docs/en/memory#how-claude-md-files-load)) — 을 크기·조상 폴더 경고와 함께 표시 |
...
`0.기능.빌드` — 겸손하게, 1.0은 아주 멀리. 가운데 숫자는 주요 기능 개선 단위에서만 천천히 움직이고, 빌드 번호는 수정·보완마다 자유롭게 올라갑니다 (기능 단위당 999까지). 공개 전 이력: 0.34.0까지는 이 체계 이전 번호이며, 홍보 전 1회에 한해 0.3.0으로 재기준선을 잡았습니다 (설치된 두 기기 수동 재설치).
