...
| 명령 | 동작 |
|------|------|
| `socrates` / `socrates list` | fzf 세션 목록. **Enter → 액션 메뉴** (`--resume` 복사 / `cd … && claude --resume …` 전체 복사 / UUID만 / 별명 / 뒤로); 단축키: **Ctrl-P 선택 세션의 프로젝트로 좁히기**, Ctrl-O cd+resume, Ctrl-Y UUID, Ctrl-N 별명. Ctrl-U/D preview 스크롤, Ctrl-T preview 확대 |
