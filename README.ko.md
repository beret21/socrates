...
| `socrates report` | 탭형 HTML 대시보드 생성+열기 (Overview / Projects / Sessions / **Config X-ray** / **Memory & Identity** / Harness). Memory & Identity는 Claude가 나를 어떻게 인식하는지(로컬 `~/.claude.json` 계정 정보)와 전 프로젝트의 자동 메모리 파일·설명을 보여줍니다. X-ray는 프로젝트별로 설정 레이어와 **CLAUDE.md 체인** — 루트→프로젝트로 내려오며 세션 시작 시 실제 로드되는 모든 메모리 파일([공식 규칙](https://code.claude.com/docs/en/memory#how-claude-md-files-load)) — 을 크기·조상 폴더 경고와 함께 표시 |
