...
`0.기능.빌드` — 겸손하게, 1.0은 아주 멀리. 가운데 숫자는 주요 기능 개선 단위에서만 천천히 움직이고, 빌드 번호는 수정·보완마다 자유롭게 올라갑니다. 공개 전 이력: 0.34.0까지는 이 체계 이전 번호이며, 홍보 전 1회에 한해 0.3.0으로 재기준선을 잡았습니다 (설치된 두 기기 수동 재설치).
