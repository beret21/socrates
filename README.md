...
| `socrates report` | generate a tabbed HTML dashboard (Overview / Projects / Sessions / **Config X-ray** / Harness) and open it. The X-ray shows, per project, the settings layers and the full **CLAUDE.md chain** — every memory file a new session would load, walking from the filesystem root down to the project ([official rule](https://code.claude.com/docs/en/memory#how-claude-md-files-load)), with sizes and ancestor-folder warnings |
