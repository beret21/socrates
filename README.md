...
| `socrates report` | generate a tabbed HTML dashboard (Overview / Projects / Sessions / **Config X-ray** / **Memory & Identity** / Harness) and open it. Memory & Identity shows how Claude identifies you (local `~/.claude.json` account info) and every project's auto-memory files with their descriptions. The X-ray shows, per project, the settings layers and the full **CLAUDE.md chain** — every memory file a new session would load, walking from the filesystem root down to the project ([official rule](https://code.claude.com/docs/en/memory#how-claude-md-files-load)), with sizes and ancestor-folder warnings |
