...
11. **Static-dashboard ceiling, measured and codified (v0.29)**: claude-mem grows ~30 observations/day at ~2.3KB each (93-day sample) → full-body embedding crosses the ~30MB comfort ceiling of a static HTML file in roughly one year. Decision: stay static (no local server) with a generation-time byte budget (`SOC_EMBED_MB`, default 25MB) — newest records embed full bodies, the rest embed titles only and are flagged; the search hint states exactly what is searched, and unlimited full-text remains in `socrates mem` (SQLite FTS scales to GBs). Also fixed: the in-dashboard observation filter now searches embedded bodies, not just titles (the earlier hint over-promised).

12. **Versioning scheme revised to `0.FEATURE.BUILD` (2026-06-12, from v0.34.0)**: the original `#.##` scheme let every fix consume a version slot (0.11→0.33 in two days — build numbers were eating version numbers). Now the middle number moves only on major feature units and the build number increments freely; `sort -V` comparison and the plugin's version-as-cache-key both handle three segments (verified, including the 9→10 rollover). Re-baselining to a lower number is forbidden once there are external users: it reads as a downgrade and would force manual reinstalls. One pre-promotion exception was executed (2026-06-12): with only the author's two machines installed, the version was reset 0.34.0 → **0.3.0** (0.1 session core / 0.2 plugin distribution / 0.3 dashboard & analysis), both machines manually reinstalled, and old tags pruned.
